...
class TestAPI(unittest.TestCase):
    def setUp(self):
        ...
        self.data_to_update = {
            'title': 'Cambio completo de titulo'
        }
    ...
    def test_get_len_task(self):
        response = self.client.get(
            path=self.path, content_type=self.content_type)
        data = json.loads(response.data.decode('utf-8'))
    ...
    def test_create_task(self):
        ...
        task_id = self.get_task_id(response)
        self.assertEquals(task_id, 4)

    def test_update_task(self):

        response = self.client.put(path=self.path_first_task, data=json.dumps(
            self.data_to_update), content_type=self.content_type)
        self.assertEqual(response.status_code, 200)
        data = json.loads(response.data.decode('utf-8'))
        title = data['data']['title']
        self.assertEquals(title, self.data_to_update['title'])
